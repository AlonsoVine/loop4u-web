  ...
  template: `
    <div class="p-4">
      <header class="flex items-center justify-between mb-4">
        <button class="px-2 py-1 text-sm rounded border border-neutral-300 dark:border-neutral-700" (click)="prev()">«</button>
        <h1 class="text-xl font-semibold">{{ headerLabel() }}</h1>
        <button class="px-2 py-1 text-sm rounded border border-neutral-300 dark:border-neutral-700" (click)="next()">»</button>
      </header>
          ...
          >
            ...
            <ng-container *ngIf="eventsByDay()[dayKey(d.date)] as items">
              <div
                ...
                [style.gridTemplateRows]="'repeat(' + (items.length > 0 ? min(items.length, 4) : 0) + ', minmax(0,1fr))'"
              >
                ...
                <ng-container *ngIf="items.length > 0">
                  ...
                  <div *ngIf="items.length > 3" class="min-h-0 overflow-hidden rounded border border-dashed border-neutral-200 dark:border-neutral-700 px-1 flex items-center text-neutral-500">… +{{ items.length - 3 }}</div>
                </ng-container>
              </div>
            </ng-container>
          </div>
...
export class CalendarPage {
  ...
  private monthStart = computed(() => new Date(Date.UTC(this.year(), this.month(), 1)).toISOString());
  private monthEnd = computed(() => new Date(Date.UTC(this.year(), this.month() + 1, 0, 23, 59, 59, 999)).toISOString());
  ...
  selectDay(d: Date) {
    this.selectedIso.set(new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate())).toISOString());
  ...
  private computeMonthEvents(startIso: string, endIso: string): Record<string, { r: Reminder; at: string }[]> {
